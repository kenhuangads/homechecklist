...
function starterTodos(itemId, install) {
  ...
  (install || []).forEach(n => { if (seen.has(n.tag) || out.length >= 3) return; seen.add(n.tag); const first = (n.text || '').split(/[；。]/)[0].trim(); if (first) out.push({ id: `${itemId}_t${out.length + 1}`, text: `${n.tag}：${first.slice(0, 40)}`, done: false }); });
  return out;
}
for (const f of files) {
  ...
  for (const cat of arr) {
    ...
    if (!it) {
      it = Object.assign({ id: cat.itemId, status: 'choosing', chosenOptionId: null, picks: [], options: opts, notes: [], todos: starterTodos(cat.itemId, cat.install), requests: [] }, fields);
...
seed.catalogVersion = 2026082301; seed.rev = (seed.rev || 0) + 1; seed.updatedAt = new Date().toISOString();
